...
export class DatePickerComponent implements OnInit {
	...
	get getCalculatedClass() {
		return `${this.selectedDates?.startDate && this.selectedDates?.endDate ? 'text-primary-400 !bg-primary-25 ' : ''}`;
	}
	...
	getDayClassNames(day: CALENDAR_DAY): string {
		const classNames = `w-full h-40px justify-center items-center transition-all duration-300 ease-in-out ${this.isCurrentDay(day.timestamp) && !this.isStartDay(day.timestamp) && !this.isEndDay(day.timestamp) ? '!bg-transparent !font-extrabold !text-primary-400' : ''} ${
			day.month > 0 ? 'hidden' : ''
		} ${this.isDayWithinStartAndEndDates(day.timestamp) ? 'bg-primary-50' : ''} ${this.isInFuture(day.timestamp) && this.formType === 'filter' ? 'opacity-30 pointer-events-none' : ''} ${
